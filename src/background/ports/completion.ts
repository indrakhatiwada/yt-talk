...
const llm = new OpenAI({
  apiKey: process.env.PLASMO_PUBLIC_OPENAI
})
...
const handler: PlasmoMessaging.PortHandler = async (req, res) => {
  ...
  try {
    ...
    completion.on("content", (delta, snapshot) => {
      ...
      res.send({ message: cumulativeData, errors: "", isEnd: false })

      completion.on("end", () => {
        res.send({ message: "END", errors: "", isEnd: true })
      })
    })
  } catch (e) {
    res.send({ message: "Something Went Wrong", errors: e })
  }
}
